...
int	ft_get_optarg(s_ft_getopt_o *opt, int argc, const char **argv)
{
	int	x;
	int	y;
...
int	ft_inser_err(s_ft_getopt_o *opt, int errn, int optopt, int errc)
{
	opt->err = errn;
	opt->opt = optopt;
	return (errc);
...
int	ft_find_next_opt(int argc, const char **argv, s_ft_getopt_o *opt, const char *optstring)
{
	int	i;
	...
	while (argv[opt->x][opt->y])
	{
		...
		if (i >= 0)
		{
			...
			if (optstring[i + 1] && optstring[i + 1] == ':')
				if (ft_get_optarg(opt, argc, argv) == -1)
				{
